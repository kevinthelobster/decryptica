  ...
  return (
    <div className="min-h-[calc(100vh-5rem)] bg-white">
      <section className="relative overflow-hidden border-b border-stone-200/70">
        ...
        <div className="relative mx-auto flex max-w-6xl flex-col gap-12 px-6 py-20 md:flex-row md:items-center md:justify-between md:py-28">
          <div className="max-w-2xl">
            <span className="inline-flex items-center gap-2  border border-red-900/20 bg-red-900/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.24em] text-red-700">
              Error 404
            </span>
            <h1 className="mt-6 font-display text-5xl font-bold tracking-tight text-stone-950 md:text-7xl">
              Signal lost,
          ...
          <div className="card-elevated max-w-md p-6 md:p-7">
            <div className="flex items-center justify-between text-sm text-stone-600">
              ...
              <span className=" border border-stone-200 bg-white px-2 py-1 text-xs text-stone-700">
                Decryptica
              </span>
            </div>
            <div className="mt-6 grid gap-4">
              {recoveryLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="group  border border-stone-200 bg-white p-4 transition-colors hover:border-red-900/40 hover:bg-white"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <h2 className="font-display text-lg font-semibold text-stone-950 transition-colors group-hover:text-red-700">
                        {link.title}
                      </h2>
                      <p className="mt-2 text-sm leading-6 text-stone-600">{link.description}</p>
                    </div>
                    <span className="mt-1 text-red-800 transition-transform group-hover:translate-x-1">
                      →
                    </span>
                  </div>
                  <div className="mt-4 text-sm font-medium text-red-700">{link.cta}</div>
                </Link>
              ))}
            </div>
          </div>
      ...
      <section className="mx-auto max-w-6xl px-6 py-12 md:py-16">
        <div className="grid gap-4  border border-stone-200 bg-white p-6 md:grid-cols-3 md:p-8">
          {[
            ...
            <div key={item} className=" border border-stone-200/80 bg-white/70 px-4 py-4 text-sm text-stone-700">
              {item}
            </div>
          ))}
        </div>
